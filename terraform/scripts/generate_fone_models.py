...
def getDriverConfidenceDict(data):
    ...
    driver_confidence_dict = dict(
        zip(driver_confidence.index, driver_confidence))
    return driver_confidence_dict


def getConstructorReliabilityDict(data):
    dnf_by_constructor = data.groupby('constructor').sum()['constructor_dnf']
    constructor_race_entered = data.groupby(
        'constructor').count()['constructor_dnf']
...
def prepareDataset(data, driver_confidence_dict, constructor_reliability_dict):
    ...
    data['constructor_reliability'] = data['constructor'].apply(
    ...
    data['active_driver'] = data['driver'].apply(
        lambda x: int(x in active_drivers))
    data['active_constructor'] = data['constructor'].apply(
...
def saveCleanedData(data):
    ...
    cleaned_data = data[['GP_name', 'quali_pos', 'constructor', 'driver', 'position',
                         'driver_confidence', 'constructor_reliability', 'active_driver', 'active_constructor']]
    cleaned_data = cleaned_data[(cleaned_data['active_driver'] == 1) & (
        cleaned_data['active_constructor'] == 1)]
    #
...
def checkProbability(data2021):
    ...
    for race in races:
        for driver in drivers:
            clst = data2021.loc[(data2021['GP_name'] == race) & (
                data2021['driver'] == driver)]['constructor'].unique().tolist()
            plst = data2021.loc[(data2021['GP_name'] == race) & (
                data2021['driver'] == driver)]['points'].unique().tolist()
            for c in clst:
                for p in plst:
    ...
    driver_points_dict = dict(
        zip(driver_points_ratio.index, driver_points_ratio))
    ...
    constructor_points_dict = dict(
        zip(constructor_pts_ratio.index, constructor_pts_ratio))
    ...
    for race in races:
        for driver in drivers:
            qlst = data2021.loc[(data2021['GP_name'] == race) & (
                data2021['driver'] == driver)]['quali_pos'].unique().tolist()
            for q in qlst:
...
def generateModels(data):
    ...
    x = data[['GP_name', 'quali_pos', 'constructor', 'driver', 'position',
              'driver_confidence', 'constructor_reliability', 'active_driver', 'active_constructor']]
    ...
    rf_random = RandomizedSearchCV(
        estimator=rf_rand, param_distributions=random_parms, n_iter=100, cv=10, verbose=0, n_jobs=-1)
    rf_random.fit(X, y)
    rf_random.best_params_
    #
    rf = RandomForestClassifier(n_estimators=100, min_samples_split=20,
                                min_samples_leaf=1, max_features='sqrt', max_depth=90, bootstrap=True)
...
def main():
    ...
    data = prepareDataset(data, driver_confidence_dict,
                          constructor_reliability_dict)
    cleaned_data = saveCleanedData(data)
    # parallel
    # Execute Threads
    t3 = Thread(target=saveDriverConfidenceDict,
                args=(driver_confidence_dict,))
    t4 = Thread(target=saveConstructorReliabilityDict,
                args=(constructor_reliability_dict,))
    ...
    print('Execution Time for HOL1 generate models: %d hour:%d min:%d sec' %
